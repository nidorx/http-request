...
/**
 * FROM: https://github.com/spring-projects/spring-framework/blob/5.1.x/spring-core/src/main/java/org/springframework/core/ParameterizedTypeReference.java
 *
 * @param <T>
 */
...
public abstract class ParameterizedTypeReference<T> {
    ...
    protected ParameterizedTypeReference() {
        ...
        if (!(type instanceof ParameterizedType)) {
            throw new RuntimeException("Type must be a parameterized type");
        }
        ParameterizedType parameterizedType = (ParameterizedType) type;
        Type[] actualTypeArguments = parameterizedType.getActualTypeArguments();
        if (actualTypeArguments.length != 1) {
            throw new RuntimeException("Number of type arguments must be 1");
        }
